...
def simulate_equity_returns(
    ...
    *,
    ...
    """Simulate a T×n demeaned equity return matrix with latent factor structure.

    Returns are generated from the model

        X = F @ B.T + E

    where F (T×k) are factor returns, B (n×k) are factor loadings, and E (T×n)
