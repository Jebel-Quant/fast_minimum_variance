...
def _problem(C, d):  # noqa: N803
    """Build a minimal Problem with identity X and custom C, d."""
    n = C.shape[0]
    return Problem(np.eye(n), C=C, d=d)
...
class TestConstraintActiveSet:
    ...
    def test_first_call_receives_empty_active_mask(self):
        ...
        p = _problem(-np.eye(3), np.zeros(3))
        first_active = []
    ...
    def test_multiple_violations_promoted_simultaneously(self):
        ...
        p = _problem(-np.eye(4), np.zeros(4))
