...
\Require Inner solver $f$;\; tolerance $\varepsilon = 10^{-6}$;\; $\alpha > 0$
  \hfill\textit{(ensures $\hat\Sigma_{\mathrm{LW},\mathcal{A}}$ non-singular)}
...
\State $\mathrm{active} \leftarrow \{1,\ldots,n\}$;\; $s \leftarrow 0$;\; $k \leftarrow 0$
\Loop
  ...
  \Else
    ...
    \State $\lambda \leftarrow \mathrm{median}_{j \in \mathrm{active}}(g_j)$
      \hfill\textit{(to solver tolerance: stationarity gives $g_j \approx \lambda$ for all $j \in \mathrm{active}$)}
...
Assume non-degeneracy: every active weight at a non-terminal primal iterate is
strictly negative for at least one violator, and at termination no excluded asset has
$g_i - \lambda = 0$.
Then Algorithm~\ref{alg:elim} terminates in finitely many outer iterations and
returns the unique global minimizer of
$\min_{w}\,f(w)$ subject to $\mathbf{1}^\top w = 1,\;w \geq 0$.
\end{theorem}

\begin{proof}
\emph{Step 1 (well-posedness of each inner solve).}
Because $\hat\Sigma_{\mathrm{LW}} = (1-\alpha)\hat\Sigma + \alpha T_0 \succ 0$,
every principal submatrix $\hat\Sigma_{\mathrm{LW},\mathcal{A}}$ is SPD.
...
Algorithm~\ref{alg:elim} exits only when (i)~all active weights are non-negative
and (ii)~$g_i \geq \lambda$ for every excluded $i \notin \mathcal{A}$.
These are the KKT conditions for the long-only problem; strict convexity of $f$
makes them sufficient for global optimality.

\emph{Step 3 (active set changes at every non-terminal iterate).}
At each outer iteration the algorithm either removes a primal violator
($|\mathcal{A}|$ strictly decreases) or adds a dual violator ($|\mathcal{A}|$
strictly increases). Hence $\mathcal{A}_{k+1} \neq \mathcal{A}_k$ for all
non-terminal $k$.

\emph{Step 4 (no active set recurs).}
Suppose $\mathcal{A}_r = \mathcal{A}_s$ for $r < s$.
Strict convexity yields a unique minimiser $w(\mathcal{A}_r)$ for this active set,
so both visits produce the same iterate.
Under non-degeneracy, the departure from $\mathcal{A}_r$ is governed by a
strictly violated constraint, which is reproduced identically on the second visit,
implying a cycle -- contradicting Step~3.
Hence no active set recurs.

\emph{Step 5 (finite termination).}
The ground set $\{1,\ldots,n\}$ has at most $2^n$ subsets; by Steps~3--4 each is
visited at most once, so termination occurs within $2^n$ outer iterations.
\end{proof}

In the experiments reported here the loop terminates in $s \leq 18$ outer
steps (Table~\ref{tab:sp500}); degenerate problems may in principle cycle,
though degeneracy is unlikely under the strictly positive shrinkage
target $\bar\lambda I$ with $\alpha > 0$.
